...
namespace GithubMonitorApp
{
    ...
    /// key on azure: Fy00tBUF/4vp0YBpys6P3sNbcKpj9MBrTjSZaHXJwiZiYhA/MIAnYA==
    /// </summary>
    public static class GithubMonitor
    {
        ...
        public static async Task<IActionResult> Run(
            ...
            ILogger log)
        {
            log.LogInformation("GITHUB Monitor Process a call.");

            string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
            var data = JsonConvert.DeserializeObject<Rootobject>(requestBody);

            //log.LogInformation(requestBody);
            log.LogInformation($"A push has been made by: {data.sender.login}, Avatar: {data.sender.avatar_url}");
...
//json from github (paste special > paste as JSON)
